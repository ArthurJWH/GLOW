...
def meltpool_geom_cal(
    ...
    for i in range(2):
        ...
        roi = patterned_image[
            y_offset : y_offset + rotated_image.shape[0],
            x_offset : x_offset + rotated_image.shape[1],
        ]
        roi_mask = rotated_mask

        # Update only the AOI regions using the mask
        np.copyto(roi, rotated_image, where=roi_mask.astype(bool))
    ...
    while True:
        ...
        for i in range(5):
            x_offset = 0
            total_height = 5 * patterned_image.shape[0]
            ...
            roi_to_check = cube_to_check[
                ...
                - layer_t * i
                + surface_mask[0 : int(rotated_mp_height) + surface_padding, :].shape[
                    0
                ],
                x_offset : x_offset + patterned_image.shape[1],
            ]
            ...
            np.copyto(
                roi_to_check,
                patterned_image[0 : int(rotated_mp_height) + surface_padding, :],
                where=surface_mask[
                    0 : int(rotated_mp_height) + surface_padding, :
                ].astype(bool),
            )

            patterned_image = np.fliplr(patterned_image)
